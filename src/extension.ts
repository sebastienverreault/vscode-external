...
function normalizePath(
  str: string,
  scope: { projectDir: string; filePath?: string; fileDir?: string },
): string {
  const { projectDir, filePath, fileDir } = scope
  return str
    .replace(/\$ProjectFileDir\$/, path.normalize(projectDir))
    .replace(/\$FilePath\$/, filePath ? path.normalize(filePath) : "")
    .replace(/\$FileDir\$/, fileDir ? path.normalize(fileDir) : "")
}
...
export function activate(context: vscode.ExtensionContext) {
  ...
  context.subscriptions.push(
    commands.registerCommand("external.add", async () => {
      ...
      const externalCommands: ICommand[] = config.get<ICommand[]>(COMMANDS_FIELD) || []
  ...
  context.subscriptions.push(
    commands.registerCommand("external.remove", async () => {
      ...
      const index: number = externalCommands.findIndex((c) => c.name === selected.label)
  ...
  context.subscriptions.push(
    commands.registerCommand("external.run", async () => {
      ...
      const scope = {
        ...
        fileDir: fileDir ? path.normalize(fileDir) : "",
      }
  ...
  context.subscriptions.push(
    commands.registerCommand("external.update", async () => {
      ...
      const targetCommand = externalCommands.find((c) => c.name === selected.label)
